...
def reset_loaded_datas():
    for key in ['loaded_datas','load_to_plot','cube_Area_SHO_l','cube_Amp_l','cube_Area_datas_l','cube_ymax_l','cube_topo_l','cube_center_l','cube_FWHM_l','cube_FWHM_datas_l','cube_Damping_l','cube_B0_l','cube_x0_l','cube_g0_l','cube_Q_l','cube_R2_l']:
...
def area_asym_computing(frequency, amplitude, n, m, cube_B0, cube_Damping, cube_FWHM, cube_f0, cube_x0, cube_center, FWHM_threshold, per_integrale):
    ...
    k,l = k[0], l[0]
    st.write("Max FWHM :", np.round(FWHM_max,2),"at coordinates : n=",k,', m=',l)
...
def SHO_plot(frequency, amplitude, test_choice, center, freq_min, freq_max, y_lim, f0_shift, FWHM_shift, window_freq, FWHM_threshold, per_integrale):
    ...
    if y[b] < y_lim : st.write("The maximal amplitude is strictly inferior to your amplitude threshold. Choose an other position.")
        ...
        try : f_maxHW, f_minHW = no_fit_FWHM(x, y, 'None', 'None')
            ...
            fig.add_traces(go.Scatter(x=frequency_interp, y=y_fit, mode='markers', name='Fit',legendrank=3))
...
def find_sym_nearest(array, value, i, j): #find the two nearest values of a symmetric curve
    ...
    y1, idx1 = find_nearest(array[:imax], value, i, j)
    y2, idx2 = find_nearest(array[imax:], value, i, j)
...
def no_fit(frequencies, cube, n, m, freq_min, freq_max, y_lim):
    ...
    for i, j in itertools.product(range(0,n,1), range(0,m,1)):
        ...
        try : no_fit_FWHM(f_interp, Ycube_interp, i, j)
        except ValueError : cube_FWHM[i, j], cube_amp[i, j], cube_f0[i, j] = np.nan, np.nan, np.nan
        else :
            f_maxHW, f_minHW = no_fit_FWHM(f_interp, Ycube_interp, i, j)
...
st.title('Force Volume AFM-IR : data cube processing')
...
with loadingTab:
    if st.session_state.FV_upload == True :
            ...
            with c2_4 :
                if st.button("Load datas", on_click=reset_loaded_datas):
                    ...
                    if st.session_state.SHO_area_load == True :
                        ...
                        elif filename+"_AREA-"+str(fit_function_load)+"_"+str(freq)+".0kHz_DThreshold_"+str(DT)+".0.txt" in load_file_list : st.session_state.cube_Area_SHO_l=np.loadtxt(LoadingPath+filename+"_AREA-"+str(fit_function_load)+"_"+str(freq)+".0kHz_DThreshold_"+str(DT)+".0.txt", dtype=np.float64, delimiter=';').reshape((st.session_state.n,st.session_state.m)); st.session_state.loaded_datas["cube_Area_SHO_l"] = True
                        elif filename+"_AIRE-"+str(fit_function_load)+"_"+str(freq)+".0kHz_DThreshold_"+str(DT)+".0.txt" in load_file_list : st.session_state.cube_Area_SHO_l=np.loadtxt(LoadingPath+filename+"_AIRE-"+str(fit_function_load)+"_"+str(freq)+".0kHz_DThreshold_"+str(DT)+".0.txt", dtype=np.float64, delimiter=';').reshape((st.session_state.n,st.session_state.m)); st.session_state.loaded_datas["cube_Area_SHO_l"] = True
                        else : toast_appearance() ; st.toast(f'{fit_function_load} area file not found.', icon="🚨", duration="infinite")
                    if st.session_state.SHO_amp_load == True :
                        ...
                        elif filename+"_AMP_"+str(freq)+".0kHz.txt" in load_file_list : st.session_state.cube_Amp_l=np.loadtxt(LoadingPath+filename+"_AMP_"+str(freq)+".0kHz.txt", dtype=np.float64, delimiter=';').reshape((st.session_state.n,st.session_state.m)); st.session_state.loaded_datas["cube_Amp_l"] = True
                        else : toast_appearance() ; st.toast(f'{fit_function_load} amplitude file not found.', icon="🚨", duration="infinite")
                    if st.session_state.datas_area_load == True :
                        if filename+"_AIRE-raw_"+str(freq)+"kHz_DThreshold_"+str(DT)+".txt" in load_file_list : st.session_state.cube_Area_datas_l=np.loadtxt(LoadingPath+filename+"_AIRE-raw_"+str(freq)+"kHz_DThreshold_"+str(DT)+".txt", dtype=np.float64, delimiter=';').reshape((st.session_state.n,st.session_state.m)); st.session_state.loaded_datas["cube_Area_datas_l"] = True
                        elif filename+"_AIRE-raw_"+str(freq)+".0kHz_DThreshold_"+str(DT)+".0.txt" in load_file_list : st.session_state.cube_Area_datas_l=np.loadtxt(LoadingPath+filename+"_AIRE-raw_"+str(freq)+".0kHz_DThreshold_"+str(DT)+".0.txt", dtype=np.float64, delimiter=';').reshape((st.session_state.n,st.session_state.m)); st.session_state.loaded_datas["cube_Area_datas_l"] = True
                        ...
                        elif filename+"_AREA-raw_"+str(freq)+".0kHz_DThreshold_"+str(DT)+".0.txt" in load_file_list : st.session_state.cube_Area_datas_l=np.loadtxt(LoadingPath+filename+"_AREA-raw_"+str(freq)+".0kHz_DThreshold_"+str(FWHM_thresh)+".0.txt", dtype=np.float64, delimiter=';').reshape((st.session_state.n,st.session_state.m)); st.session_state.loaded_datas["cube_Area_datas_l"] = True
                    ...
                    if st.session_state.central_freq_load == True :
                        ...
                        elif filename+"_F0_"+str(freq)+".0kHz.txt" in load_file_list : st.session_state.cube_center_l=np.loadtxt(LoadingPath+filename+"_F0_"+str(freq)+".0kHz.txt", dtype=np.float64, delimiter=';').reshape((st.session_state.n,st.session_state.m)); st.session_state.loaded_datas["cube_center_l"] = True
                        else : toast_appearance() ; st.toast(f'{fit_function_load} central frequency file not found.', icon="🚨", duration="infinite")
                    if st.session_state.SHO_FWHM_load == True :
                        ...
                        elif filename+"_FWMH_"+str(freq)+".0kHz.txt" in load_file_list : st.session_state.cube_FWHM_l=np.loadtxt(LoadingPath+filename+"_FWMH_"+str(freq)+".0kHz.txt", dtype=np.float64, delimiter=';').reshape((st.session_state.n,st.session_state.m)); st.session_state.loaded_datas["cube_FWHM_l"] = True
                        else : toast_appearance() ; st.toast(f'{fit_function_load} FWHM file not found.', icon="🚨", duration="infinite")
                    if st.session_state.datas_FWHM_load == True :
                        if filename+"_FWHM-datas_"+str(freq)+"kHz.txt" in load_file_list : st.session_state.cube_FWHM_datas_l=np.loadtxt(LoadingPath+filename+"_FWHM-datas_"+str(freq)+"kHz.txt", dtype=np.float64, delimiter=';').reshape((st.session_state.n,st.session_state.m)); st.session_state.loaded_datas["cube_FWHM_datas_l"] = True
                        else : toast_appearance() ; st.toast('Datas FWHM file not found.', icon="🚨", duration="infinite")
                    if st.session_state.SHO_damping_load == True :
                        ...
                        elif filename+"_DAMPING_"+str(freq)+".0kHz.txt" in load_file_list : st.session_state.cube_Damping_l=np.loadtxt(LoadingPath+filename+"_DAMPING_"+str(freq)+".0kHz.txt", dtype=np.float64, delimiter=';').reshape((st.session_state.n,st.session_state.m)); st.session_state.loaded_datas["cube_Damping_l"] = True
                    ...
                    if st.session_state.SHO_B0_load == True :
                        ...
                        elif filename+"_B0_"+str(freq)+".0kHz.txt" in load_file_list : st.session_state.cube_B0_l=np.loadtxt(LoadingPath+filename+"_B0_"+str(freq)+".0kHz.txt", dtype=np.float64, delimiter=';').reshape((st.session_state.n,st.session_state.m)); st.session_state.loaded_datas["cube_B0_l"] = True
                        else : toast_appearance() ; st.toast(f'{fit_function_load} B0 file not found.', icon="🚨", duration="infinite")
        ...
        st.session_state.load_to_plot=[i for i in st.session_state.loaded_datas if st.session_state.loaded_datas[i]==True]
        with st.expander('General parameters of the results figures') :
        ...
        if sum(st.session_state.loaded_datas.values())>=2 :
            ...
            if 'cube_ymax_l' in st.session_state : st.session_state.plot_params_cube_ymax_l = {'cube':st.session_state.cube_ymax_l, 'color_index':38, 'map_origin':st.session_state.map_origin_load, 'colorbar_label':"Amplitude (mV)", 'title':"Datas' max amplitude", 'results_width':st.session_state.results_width_load, 'results_height':st.session_state.results_height_load,  'bins_width_initial':5, 'n':st.session_state.n, 'm':st.session_state.m, 'key':'ax4_l', 'x':st.session_state.x, 'y':st.session_state.y, 'xy_unit':st.session_state.xy_unit, 'scale_units':st.session_state.map_load}
            ...
            if 'cube_FWHM_datas_l' in st.session_state : st.session_state.plot_params_cube_FWHM_datas_l = {'cube':st.session_state.cube_FWHM_datas_l, 'color_index':60, 'map_origin':st.session_state.map_origin_load, 'colorbar_label':"Frequency (kHz)", 'title':"Datas' FWHM", 'results_width':st.session_state.results_width_load, 'results_height':st.session_state.results_height_load,  'bins_width_initial':1, 'n':st.session_state.n, 'm':st.session_state.m, 'key':'ax8_l', 'x':st.session_state.x, 'y':st.session_state.y, 'xy_unit':st.session_state.xy_unit, 'scale_units':st.session_state.map_load}
            if 'cube_Damping_l' in st.session_state : st.session_state.plot_params_cube_Damping_l = {'cube':st.session_state.cube_Damping_l, 'color_index':60, 'map_origin':st.session_state.map_origin_load, 'colorbar_label':"Frequency (kHz)", 'title':"Damping SHO function", 'results_width':st.session_state.results_width_load, 'results_height':st.session_state.results_height_load,  'bins_width_initial':1, 'n':st.session_state.n, 'm':st.session_state.m, 'key':'ax9_l', 'x':st.session_state.x, 'y':st.session_state.y, 'xy_unit':st.session_state.xy_unit, 'scale_units':st.session_state.map_load}
            if 'cube_B0_l' in st.session_state : st.session_state.plot_params_cube_B0_l = {'cube':st.session_state.cube_B0_l, 'color_index':10, 'map_origin':st.session_state.map_origin_load, 'colorbar_label':"B0", 'title':"B0 SHO function", 'results_width':st.session_state.results_width_load, 'results_height':st.session_state.results_height_load,  'bins_width_initial':1, 'n':st.session_state.n, 'm':st.session_state.m, 'key':'ax10_l', 'x':st.session_state.x, 'y':st.session_state.y, 'xy_unit':st.session_state.xy_unit, 'scale_units':st.session_state.map_load}
            if 'cube_x0_l' in st.session_state : st.session_state.plot_params_cube_x0_l = {'cube':st.session_state.cube_x0_l, 'color_index':10, 'map_origin':st.session_state.map_origin_load, 'colorbar_label':"x0", 'title':"x0 asymetric SHO function", 'results_width':st.session_state.results_width_load, 'results_height':st.session_state.results_height_load,  'bins_width_initial':1, 'n':st.session_state.n, 'm':st.session_state.m, 'key':'ax11_l', 'x':st.session_state.x, 'y':st.session_state.y, 'xy_unit':st.session_state.xy_unit, 'scale_units':st.session_state.map_load}
            if 'cube_g0_l' in st.session_state : st.session_state.plot_params_cube_g0_l = {'cube':st.session_state.cube_g0_l, 'color_index':10, 'map_origin':st.session_state.map_origin_load, 'colorbar_label':"g0", 'title':"g0 asymetric SHO function", 'results_width':st.session_state.results_width_load, 'results_height':st.session_state.results_height_load,  'bins_width_initial':1, 'n':st.session_state.n, 'm':st.session_state.m, 'key':'ax12_l', 'x':st.session_state.x, 'y':st.session_state.y, 'xy_unit':st.session_state.xy_unit, 'scale_units':st.session_state.map_load}            
            if 'cube_Q_l' in st.session_state : st.session_state.plot_params_cube_Q_l = {'cube':st.session_state.cube_Q_l, 'color_index':10, 'map_origin':st.session_state.map_origin_load, 'colorbar_label':"Q factor", 'title':"Q factor", 'results_width':st.session_state.results_width_load, 'results_height':st.session_state.results_height_load,  'bins_width_initial':1, 'n':st.session_state.n, 'm':st.session_state.m, 'key':'ax13_l', 'x':st.session_state.x, 'y':st.session_state.y, 'xy_unit':st.session_state.xy_unit, 'scale_units':st.session_state.map_load}
            if 'cube_R2_l' in st.session_state : st.session_state.plot_params_cube_R2_l = {'cube':st.session_state.cube_R2_l, 'color_index':34, 'map_origin':st.session_state.map_origin_load, 'colorbar_label':"R²", 'title':"R² fit score", 'results_width':st.session_state.results_width_load, 'results_height':st.session_state.results_height_load,  'bins_width_initial':0.005, 'n':st.session_state.n, 'm':st.session_state.m, 'key':'ax14_l', 'x':st.session_state.x, 'y':st.session_state.y, 'xy_unit':st.session_state.xy_unit, 'scale_units':st.session_state.map_load}
            k=iter(st.session_state.load_to_plot)
